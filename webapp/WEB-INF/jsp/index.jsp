...
<aos:body>
	...
	<%-- Center导航 --%>
	<div id="div_center">
		<iframe id="iframe_main" src="do?router=homeService.initPortal"></iframe>
	</div>
	<%-- Banner导航 --%>
...
<script type="text/javascript">
	...
	function fnaddtab(url, menuname, module_id, root_id) {
		...
		url = url.indexOf('http://') === 0 ? url : (url.indexOf('/')===0 ? ('${cxt}'+url) : '${cxt}/http/do?router=' + url);
		var index = url.indexOf('?');
		//一级菜单的主页面所属的页面元素其pageid_同moduleid_。
		url = url + (index === -1 ? '?' : '&') + 'aos_module_id=' + module_id;
	...
	function fn_logout() {
		AOS.confirm('注销并安全退出系统吗？', function(btn) {
			...
			AOS.ajax({
				...
				ok : function(data) {
					AOS.unmask();
					window.location.href = 'do?router=homeService.initLogin';
				}
			});
		});
	}
