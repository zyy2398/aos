...
<aos:onready>
	<aos:viewport layout="border">
		...
		<aos:gridpanel id="g_module" url="moduleService.listModules" region="center" onitemdblclick="w_module_u_show"
			...
			<aos:column header="权限标识" dataIndex="perms" width="250" celltip="true" />
			<aos:column header="类型" dataIndex="menu_type" fixedWidth="60" align="center" rendererFn="menu_type_tpl" />
	...
	<aos:window id="w_module" title="新增功能模块">
		<aos:formpanel id="f_module" width="400" layout="anchor" labelWidth="65">
			...
			<aos:radioboxgroup fieldLabel="菜单类型" columns="3" onchange="select_type">
				<aos:radiobox name="menu_type" boxLabel="目录" inputValue="M" />
				<aos:radiobox name="menu_type" boxLabel="菜单" inputValue="C" checked="true"/>
				<aos:radiobox name="menu_type" boxLabel="按钮" inputValue="F" />
			</aos:radioboxgroup>
			...
			<aos:textfield name="perms" fieldLabel="权限标识" maxLength="100" />
	...
	<aos:window id="w_module_u" title="修改功能模块">
		<aos:formpanel id="f_module_u" width="400" layout="anchor" labelWidth="65">
			...
			<aos:radioboxgroup fieldLabel="菜单类型" columns="3" onchange="select_type_u">
				<aos:radiobox name="menu_type" boxLabel="目录" inputValue="M" />
				<aos:radiobox name="menu_type" boxLabel="菜单" inputValue="C" />
				<aos:radiobox name="menu_type" boxLabel="按钮" inputValue="F" />
			</aos:radioboxgroup>
			<aos:textfield name="name" fieldLabel="模块名称" allowBlank="false" maxLength="50" />
			<aos:textfield name="url" fieldLabel="请求地址" maxLength="100" />
	...
	<script type="text/javascript">
		...
		function select_type(me,newValue){
			var type = newValue.menu_type;
			if(type=='F'){
				AOS.hides(f_module,"url,is_auto_expand,icon_name,vector,is_enable");
			}else{
				AOS.shows(f_module,"url,is_auto_expand,icon_name,vector,is_enable");
			}
		}

		function menu_type_tpl(value){
			if(value=='M'){
				return "<button class='cbtn'>目录</button>";
			}else if(value=='F'){
				return "<button class='cbtn_danger'>按钮</button>"
			}else if(value=='C'){
				return "<button class='cbtn_warn'>菜单</button>";
			}
		}

		function select_type_u(me,newValue){
			var type = newValue.menu_type;
			if(type=='F'){
				AOS.hides(f_module_u,"url,is_auto_expand,icon_name,vector,is_enable");
			}else{
				AOS.shows(f_module_u,"url,is_auto_expand,icon_name,vector,is_enable");
			}
		}
