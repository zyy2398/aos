...
<aos:onready>
	<aos:viewport layout="fit">
		<aos:gridpanel id="g_role" url="roleService.listRole" onrender="g_role_query" forceFit="true"
			...
			<aos:column header="角色名称" dataIndex="name" celltip="true" width="100" />
			...
			<aos:column header="数据权限" dataIndex="data_scope" hidden="true"/>
			...
			<aos:column header="功能权限" rendererFn="_btn_grant_render" align="center" fixedWidth="80" />
			<aos:column header="数据权限" rendererFn="_btn_data_render" align="center" fixedWidth="80" />
	...
	<aos:window id="w_role_u" title="修改角色">
		<aos:formpanel id="f_role_u" width="500" labelWidth="65" layout="anchor">
	...
	<aos:window id="w_role_org_u" title="数据权限" height="600" width="500" layout="column" onshow="w_role_org_onshow">
		<aos:formpanel id="f_role_org_u" labelWidth="65" columnWidth="1" layout="anchor">
			<aos:hiddenfield name="id" />
			<aos:textfield name="name" fieldLabel="角色名称" readOnly="true" allowBlank="false" maxLength="50" />
			<aos:combobox name="type" fieldLabel="角色类型" readOnly="true" allowBlank="false" dicField="role_type" />
			<aos:combobox name="data_scope" fieldLabel="数据权限" allowBlank="false" onselect="select_org">
				<aos:option value="1" display="全部数据权限" />
				<aos:option value="2" display="自定数据权限" />
				<aos:option value="3" display="本部门数据权限" />
				<aos:option value="4" display="本部门及以下数据权限" />
				<aos:option value="5" display="仅本人数据权限" />
			</aos:combobox>
		</aos:formpanel>
		<aos:treepanel id="org_data_tree" columnWidth="1" height="-250" title="自定数据权限" singleClick="false" cascade="true" rootVisible="false"
					   url="orgService.getCheckboxTreeData" margin="5" border="true"/>
		<aos:docked dock="bottom" ui="footer">
			<aos:dockeditem xtype="tbfill" />
			<aos:dockeditem onclick="f_role_org_u_save" text="保存" icon="ok.png" />
			<aos:dockeditem onclick="#w_role_org_u.hide();" text="关闭" icon="close.png" />
		</aos:docked>
	</aos:window>

	<aos:window id="w_role_module" title="功能权限" width="600" height="-10" layout="border" onshow="w_role_module_onshow">
		<aos:treepanel id="t_module" rootText="经办权限[导航菜单]" width="300" headerBorder="0 1 0 0" bodyBorder="0 1 0 0"
			url="roleService.getTreeData" singleClick="false" rootIcon="icon140.png" cascade="false" rootExpanded="false"
		...
		<aos:treepanel id="t_module_admin" rootText="管理权限[授权时待选菜单]" width="300" bodyBorder="0 0 0 0"
			url="roleService.getAdminTreeData" singleClick="false" rootIcon="icon146.png" cascade="false" rootExpanded="false"
	 	...
	 	function _role_user_cancel(){
	       ...
	       //选择用户按钮列转换
			...
			function _btn_grant_render(value, metaData, record, rowIndex, colIndex,store) {
				return '<input type="button" value="功能权限" class="cbtn_danger" onclick="_btn_grant_onclick();" />';
			}

			//授权按钮列转换
			function _btn_data_render(value, metaData, record, rowIndex, colIndex,store) {
				return '<input type="button" value="数据权限" class="cbtn_warn" onclick="_btn_data_onclick();" />';
			}
            ...
            //弹出修改数据权限窗口
            function w_role_org_onshow(){
				var record = AOS.selectone(g_role);
				console.dir(record);
				if (record) {
					lond_org_data();
					if(record.data.data_scope == '2'){
						org_data_tree.show();
					}else{
						org_data_tree.hide();
					}
					f_role_org_u.loadRecord(record);
				}
			}

            function lond_org_data(){
				refreshnode = org_data_tree.getRootNode();
				var record = AOS.selectone(g_role);
				var params = {
					role_id : record.data.id
				};
				org_data_tree_store.getProxy().extraParams = params;
				org_data_tree_store.load();
			}
			...
			//数据权限保存
			function f_role_org_u_save() {
				AOS.ajax({
					params:{
						org_rows: AOS.selection(org_data_tree, 'id')
					},
					forms: f_role_org_u,
					url : 'roleService.saveRoleOrgInfo',
					ok : function(data) {
						w_role_org_u.hide();
						g_role_store.reload();
						AOS.tip(data.appmsg);
            ...
            function select_org(me, records){
				if(me.getValue()=='2'){
					lond_org_data();
					org_data_tree.show();
				}else{
...
<script type="text/javascript">
   ...
   function _btn_data_onclick(){
	   Ext.getCmp('w_role_org_u').show();
   }
