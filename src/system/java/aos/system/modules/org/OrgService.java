...
public class OrgService {
	...
	public void getCheckboxTreeData(HttpModel httpModel) {
		...
		List<Dto> aosOrgPOs = sqlDao.list("Org.findOrgRole",qDto);
		for (Dto aosOrgPO : aosOrgPOs) {
			TreeNode treeNode = new TreeNode();
			treeNode.setId(aosOrgPO.getString("id"));
			treeNode.setText(aosOrgPO.getString("name"));
			treeNode.setParentId(aosOrgPO.getString("parent_id"));
			treeNode.setIcon(aosOrgPO.getString("icon_name"));
			//这个决定是否在节点上初选复选框，true为初始选中
			treeNode.setChecked(StringUtils.equals(aosOrgPO.getString("checked"), SystemCons.IS.YES)? false : true);
			treeNode.setLeaf(StringUtils.equals(aosOrgPO.getString("is_leaf"), SystemCons.IS.YES) ? true : false );
			treeNode.setExpanded(StringUtils.equals(aosOrgPO.getString("is_auto_expand"), SystemCons.IS.YES) ? true : false );
