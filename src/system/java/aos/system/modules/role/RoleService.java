...
public class RoleService {
	...
	@Transactional
	public void saveRoleOrgInfo(HttpModel httpModel){
		Dto inDto = httpModel.getInDto();
		String org_rows = inDto.getString("org_rows");
		Integer role_id = inDto.getInteger("id");
		AosRolePO aosRolePO = new AosRolePO();
		aosRolePO.copyProperties(inDto);
		sqlDao.delete("Role.deleteRoleOrg",Dtos.newDto("role_id",role_id));
		if(StringUtils.equals(aosRolePO.getData_scope(),"2")){
			for (String org_id : StringUtils.split(org_rows, ",")) {
				Dto roleOrgDto = Dtos.newDto();
				roleOrgDto.put("role_id",role_id);
				roleOrgDto.put("org_id",org_id);
				sqlDao.update("Role.insertRoleOrg",roleOrgDto);
			}
		}
		aosRoleDao.updateByKey(aosRolePO);
