...
import aos.framework.taglib.core.model.vo.ConfigVO;
...
public class ContainerTagSupport extends ComponentTagSupport {
	 ...
	 * @return
	 */
	public String getMyOnshow() {
		if (AOSUtils.isEmpty(onshow)) {
			return onshow;
		}

		if (StringUtils.isBlank(onshow)) {
			return null;
		}

		// 执行JS代码
		if (onshow.indexOf("#") != -1) {
			onshow = StringUtils.substring(onshow, 1);
			onshow = "function(){" + onshow + "}";
			return onshow;
		}

		if (onshow.indexOf("!") != -1) {
			return onshow.substring(1, onshow.length());
		}
